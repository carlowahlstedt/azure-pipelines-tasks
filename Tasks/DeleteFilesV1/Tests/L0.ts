...
describe('DeleteFiles L0 Suite', function () {
    before(() => { });

    after(() => { });

    it('delete files from srcdir to destdir', (done: MochaDone) => {
        this.timeout(1000);

        let testPath = path.join(__dirname, 'L0deleteAllFiles.js');
        ...
        assert(
            runner.stdOutContained(`deleting ${path.normalize('/srcDir/someOtherDir/file1.file')} to ${path.normalize('/destDir/someOtherDir/file1.file')}`),
            'should have copied dir1 file1');
        assert(
            runner.stdOutContained(`deleting ${path.normalize('/srcDir/someOtherDir/file2.file')} to ${path.normalize('/destDir/someOtherDir/file2.file')}`),
            'should have copied dir1 file2');
        assert(
            runner.stdOutContained(`deleting ${path.normalize('/srcDir/someOtherDir2/file1.file')} to ${path.normalize('/destDir/someOtherDir2/file1.file')}`),
            'should have copied dir2 file1');
        assert(
            runner.stdOutContained(`deleting ${path.normalize('/srcDir/someOtherDir2/file2.file')} to ${path.normalize('/destDir/someOtherDir2/file2.file')}`),
            'should have copied dir2 file2');
        assert(
            runner.stdOutContained(`deleting ${path.normalize('/srcDir/someOtherDir2/file3.file')} to ${path.normalize('/destDir/someOtherDir2/file3.file')}`),
            'should have copied dir2 file3');
        done();
    });

    // it('delete files and subtract based on exclude pattern', (done: MochaDone) => {
    //     this.timeout(1000);

    //     let testPath = path.join(__dirname, 'L0deleteSubtractExclude.js');
    //     let runner: mocktest.MockTestRunner = new mocktest.MockTestRunner(testPath);
    //     runner.run();
